...
#include "UnityCG.cginc"  
	...
	fixed4 frag_dof(v2f_dof i) : SV_Target
	{
		...
		//fixed4 final = (depth <= _focalDistance) ? original : lerp(original, blur, clamp((depth - _focalDistance), 0, 1));
		//上面的结果，再进行一次计算，如果depth大于焦点的物体，使用上面的结果和模糊图像差值，得到近景模糊效果  
		//final = (depth > _focalDistance) ? final : lerp(original, blur, clamp((_focalDistance - depth), 0, 1));
		...
		float focal01 = clamp(sign(depth - _focalDistance),0,1);  
		fixed4 final = (1 - focal01) * original + focal01 * lerp(original, blur, clamp((depth - _focalDistance), 0, 1));
		final = (focal01)* final + (1 - focal01) * lerp(original, blur, clamp((_focalDistance - depth), 0, 1));
		return final;
	}
